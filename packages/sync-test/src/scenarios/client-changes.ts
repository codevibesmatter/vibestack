...
class ClientChangesTester extends SyncTester {
      ...
      if ('type' in message) {
        switch (message.type) {
          ...
          case 'srv_lsn_update':
            ...
            console.log(`📩 RECEIVED: srv_lsn_update (LSN: ${this.lastLSN})`);
            break;
          
          case 'srv_sync_completed':
            console.log(`📩 RECEIVED: srv_sync_completed (sync complete notification)`);
            break;
          
          case 'srv_error':
            console.error(`❌ RECEIVED: srv_error (server reported an error)`);
            break;
          
          case 'srv_send_changes':
            const changes = (message as any).changes?.length || 0;
            console.log(`📩 RECEIVED: srv_send_changes (${changes} changes)`);
            break;
          
          case 'srv_init_start':
            console.warn(`⚠️ RECEIVED: srv_init_start - this means we're in initial sync mode`);
            break;
          
          case 'srv_init_changes':
            console.warn(`⚠️ RECEIVED: srv_init_changes - this means we're in initial sync mode`);
            break;
          
          case 'srv_init_complete':
            console.warn(`⚠️ RECEIVED: srv_init_complete - initial sync is complete`);
            break;
  ...
  public async sendSingleChange(): Promise<void> {
    try {
      // Generate a single change for the Task entity
      const change = await generateSingleChange(Task);
  ...
  /**
   * Send multiple changes to the server
   */
  public async sendBulkChanges(count: number = 5): Promise<void> {
    try {
      // Generate multiple changes for the Task entity
      const changes = await generateBulkChanges(Task, count);
      ...
      const changeIds = changes.map(c => c.data.id).join(', ');
...
async function testClientChanges() {
  ...
  // Setup timeouts
  const timeoutDuration = 60000; // 60 seconds - longer timeout for server processing
  ...
  try {
    ...
    // Wait for change to be processed with timeout - much longer wait
    console.log('Waiting for server to process change...');
    const singleChangeTimeout = Date.now() + DEFAULT_CONFIG.changeWaitTime * 3;
    while (tester.getChangeStats().received === 0 && Date.now() < singleChangeTimeout) {
      // Print a waiting indicator every second
      console.log('Waiting for server response...');
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
    
    // Verify single change was acknowledged
    if (tester.getChangeStats().received === 0) {
      console.warn('Warning: Single change was not acknowledged by server within timeout');
    } else {
      console.log('Single change was acknowledged by server');
    }
    
    // STEP 6: Test bulk changes
    console.log('\nTesting bulk changes...');
    const initialReceived = tester.getChangeStats().received;
    const initialApplied = tester.getChangeStats().applied;
    
    // Send bulk changes
    await tester.sendBulkChanges(3); // Reduce to 3 changes instead of 5 to lower processing load
    
    // Wait for changes to be processed with timeout - much longer wait
    console.log('Waiting for server to process bulk changes...');
    const bulkChangeTimeout = Date.now() + DEFAULT_CONFIG.changeWaitTime * 5; // longer timeout
    
    let receivedBulk = false;
    let appliedBulk = false;
    
    while (Date.now() < bulkChangeTimeout && (!receivedBulk || !appliedBulk)) {
      // Check if we've received acknowledgments
      const currentStats = tester.getChangeStats();
      
      // If we haven't seen 'received' acknowledgment yet
      if (!receivedBulk && currentStats.received > initialReceived) {
        receivedBulk = true;
        console.log(`✅ Server acknowledged receipt of bulk changes`);
      }
      
      // If we haven't seen 'applied' acknowledgment yet
      if (!appliedBulk && currentStats.applied > initialApplied) {
        appliedBulk = true;
        console.log(`✅ Server acknowledged application of bulk changes`);
      }
      
      // If we're still waiting for either acknowledgment
      if (!receivedBulk || !appliedBulk) {
        console.log('Waiting for server response...');
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    }
    
    // Report on bulk change results
    if (!receivedBulk) {
      console.warn('⚠️ Warning: Bulk changes were not acknowledged as received by server within timeout');
    }
    
    if (!appliedBulk) {
      console.warn('⚠️ Warning: Bulk changes were not acknowledged as applied by server within timeout');
    }
